...
export default function Portfolio() {
  ...
  const commandNodes = [
    ...
    { label: 'Chronicles', action: () => navigate('/blog') },
    { label: 'About', action: () => scrollToSection('about-me') },
  ...
  return (
    <div className="relative bg-[#050505] min-h-screen overflow-x-hidden">
      ...
      >
        ...
        <div className="absolute inset-0 z-[1] bg-gradient-to-b from-[#05070a]/28 via-transparent to-[#060607]/35" aria-hidden />

        {/* Lightning/energy lines across scene */}
        <div className="pointer-events-none absolute inset-0 z-[5]" aria-hidden>
          {[...Array(6)].map((_, i) => (
            <motion.div
              key={i}
              className="absolute h-px bg-gradient-to-r from-transparent via-[#d9b056]/65 to-transparent"
              style={{
                top: `${18 + i * 10}%`,
                left: `${-8 + i * 4}%`,
                width: `${72 - i * 6}%`,
        ...
        {/* Faint particles around moon area */}
        <div className="pointer-events-none absolute inset-0 z-[6]" aria-hidden>
          {[...Array(10)].map((_, i) => (
            <motion.span
              key={i}
              className="absolute w-1 h-1 rounded-full bg-[#e2c979]/60"
              style={{
                top: `${10 + (i % 5) * 6}%`,
                right: `${17 + (i % 4) * 5}%`,
              }}
              animate={{ y: [0, -14, 0], opacity: [0.2, 0.8, 0.2] }}
              transition={{ duration: 2.6 + (i % 3), repeat: Infinity, delay: i * 0.25, ease: 'easeInOut' }}
            />
          ))}
        </div>
        ...
        <motion.div
          ...
          className="absolute z-10 left-1/2 top-[64%] md:top-[70%] -translate-x-1/2 -translate-y-1/2 w-full max-w-4xl px-4"
        >
          <div className="relative mx-auto max-w-3xl rounded-2xl border border-[#e0bd66]/25 bg-[#0a0d12]/45 backdrop-blur-md px-4 md:px-8 py-5 md:py-8 text-center">
            <div className="pointer-events-none absolute inset-0 rounded-2xl bg-[radial-gradient(circle_at_center,rgba(229,192,105,0.14),transparent_60%)]" />

            <p className="relative font-mono tracking-[0.18em] text-sm md:text-base text-[#f0d58f]">
              WELCOME TO NERGAL LABS
            </p>
            <p className="relative mt-2 text-xs md:text-sm text-[#e4e0d1]">
              Creative Systems • AI • Digital Experiences
            </p>

            <div className="relative mt-5 md:mt-6 grid grid-cols-2 md:grid-cols-4 gap-2.5 md:gap-3">
              {commandNodes.map((node) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="group relative rounded-xl border border-[#dfbe68]/30 bg-[#0b1018]/65 px-3 md:px-4 py-2.5 md:py-3 text-sm font-mono text-[#f8f2dc] transition-colors hover:border-[#f1d07e] hover:bg-[#101824]/75"
                >
                  <span className="absolute left-2 top-1 text-[10px] text-[#d8b557]/70">◌</span>
                  <span className="absolute right-2 bottom-1 text-[10px] text-[#d8b557]/70">◌</span>
                  <span>{node.label}</span>
                  <motion.span
                    className="pointer-events-none absolute inset-0 rounded-xl"
                    animate={{ boxShadow: ['0 0 0 rgba(232,190,87,0)', '0 0 16px rgba(232,190,87,0.2)', '0 0 0 rgba(232,190,87,0)'] }}
        ...
        <button
          ...
          className="absolute z-20 right-[16%] md:right-[18%] top-[14svh] md:top-[12svh] w-14 h-14 md:w-16 md:h-16 rounded-full"
        ...
        >
          <motion.span
            ...
            animate={{
              boxShadow: [
                '0 0 0 2px rgba(255,215,0,0.35), 0 0 18px rgba(255,215,0,0.35)',
                '0 0 0 2px rgba(255,215,0,0.55), 0 0 26px rgba(255,215,0,0.45)',
                '0 0 0 2px rgba(255,215,0,0.35), 0 0 18px rgba(255,215,0,0.35)',
              ],
            }}
            transition={{ duration: 2.4, repeat: Infinity, ease: 'easeInOut' }}
          />
          <span className="absolute inset-0 rounded-full bg-gradient-to-br from-[#FFD700]/35 to-[#8B0000]/20 backdrop-blur-[1px]" />
          <span className="absolute inset-[22%] rounded-full border border-[#FFD700]/60" />
        </button>
